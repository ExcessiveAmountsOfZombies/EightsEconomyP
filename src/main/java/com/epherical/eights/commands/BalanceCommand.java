...
public class BalanceCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralCommandNode<CommandSourceStack> mainCommand = dispatcher.register(Commands.literal("bal")
                .requires(commandSourceStack -> Permissions.check(commandSourceStack, "eights.command.balance.check"))
                .executes(context -> checkBalance(context, context.getSource().getPlayerOrException()))
                .then(Commands.literal("add")
                        .requires(commandSourceStack -> Permissions.check(commandSourceStack, "eights.command.balance.add"))
                        .then(Commands.argument("player", EntityArgument.players())
                                .then(Commands.argument("amount", IntegerArgumentType.integer(1))
                                        .executes(BalanceCommand::addMoney))))
                .then(Commands.literal("remove")
                        .requires(commandSourceStack -> Permissions.check(commandSourceStack, "eights.command.balance.remove"))
                        .then(Commands.argument("player", EntityArgument.players())
                                .then(Commands.argument("amount", IntegerArgumentType.integer(1))
                                        .executes(BalanceCommand::removeMoney))))
                .then(Commands.literal("set")
                        .requires(commandSourceStack -> Permissions.check(commandSourceStack, "eights.command.balance.set"))
                        .then(Commands.argument("player", EntityArgument.players())
                                .then(Commands.argument("amount", IntegerArgumentType.integer(1))
                                        .executes(BalanceCommand::setMoney))))
                .then(Commands.literal("pay")
                        .requires(commandSourceStack -> Permissions.check(commandSourceStack, "eights.command.balance.pay"))
                        .then(Commands.argument("player", EntityArgument.players())
                                .then(Commands.argument("amount", IntegerArgumentType.integer(1))
